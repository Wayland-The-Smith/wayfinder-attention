#!/usr/bin/env python3
"""
slot_pointer fixed grid @ T=2048 — pool_mlp_token readout (query-gated position mix → vocab).

Usage:
  python run_slot_pointer_fixed_grid_pool_mlp_suite.py --dry-run
  python run_slot_pointer_fixed_grid_pool_mlp_suite.py
"""
...
DEFAULT_CONFIG = ROOT / "configs" / "routing_slot_pointer_fixed_grid_t2048_50q_pool_mlp_token.yaml"
DEFAULT_OUTPUT = ROOT / "experiments" / "Experiment_7" / "slot_pointer_fixed_grid_pool_mlp_t2048_50q"
...
def preflight(dry_run: bool) -> dict:
    ...
    print("=== slot_pointer fixed-grid pool_mlp_token preflight ===")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="slot_pointer fixed grid 50q — pool_mlp_token (dense_flash)"
    )
    parser.add_argument("--dry-run", action="store_true")
    parser.add_argument("--config", type=Path, default=DEFAULT_CONFIG)
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--n-layers", type=int, default=None)
    parser.add_argument("--pool-positions", type=int, default=None, help="override pool_mlp_positions K")
    ...
    if args.pool_positions is not None:
        arena_cfg.setdefault("model", {})["pool_mlp_positions"] = int(args.pool_positions)
    train_t = int(arena_cfg["train_context_length"])
    n_layers = args.n_layers or int(arena_cfg.get("n_layers", 2))
    ...
    print("=== slot_pointer fixed-grid pool_mlp_token plan ===")
    print(f"  task=slot_pointer  placement={bench.get('slot_quad_placement')}  quads={bench.get('num_slot_quads', 50)}")
    print(f"  T={train_t}  n_layers={n_layers}  output_head={model_cfg.get('output_head')}")
    print(f"  pool_mlp_positions={model_cfg.get('pool_mlp_positions')}")
    ...
    log = logging.getLogger("slot_pointer_fixed_grid_pool_mlp.train")
    ...
    summary = {
        ...
        "kind": "slot_pointer_fixed_grid_pool_mlp_suite",
        "dry_run": args.dry_run,
        "train_context_length": train_t,
        "n_layers": n_layers,
        "output_head": model_cfg.get("output_head"),
        "pool_mlp_positions": model_cfg.get("pool_mlp_positions"),
