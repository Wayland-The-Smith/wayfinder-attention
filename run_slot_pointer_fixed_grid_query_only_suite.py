#!/usr/bin/env python3
"""
slot_pointer @ T=2048 — fixed-grid 50 quads, query-only value token (lm_token).

No pointer_mlp head: model sees queried address at T-1 and must emit the matching
value token id from that position's logits (harder than fixed-grid + pointer_mlp).

Usage:
  python run_slot_pointer_fixed_grid_query_only_suite.py --dry-run
  python run_slot_pointer_fixed_grid_query_only_suite.py
"""
...
DEFAULT_CONFIG = ROOT / "configs" / "routing_slot_pointer_fixed_grid_t2048_50q_query_only.yaml"
DEFAULT_OUTPUT = ROOT / "experiments" / "Experiment_7" / "slot_pointer_fixed_grid_query_only_t2048_50q"
...
def preflight(dry_run: bool) -> dict:
    ...
    print("=== slot_pointer fixed-grid query-only preflight ===")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="slot_pointer fixed grid 50q — query-only value token (dense_flash)"
    )
    ...
    print("=== slot_pointer fixed-grid query-only plan ===")
    ...
    log = logging.getLogger("slot_pointer_fixed_grid_query_only.train")
    ...
    summary = {
        ...
        "kind": "slot_pointer_fixed_grid_query_only_suite",
